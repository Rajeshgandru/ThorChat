...
public class PUGIFLoading
{
    ...
    public func hide() {
        ...
        {
            NotificationCenter.default.removeObserver(self, name: .NSUbiquityIdentityDidChange ,object: nil)
    ...
    public func show(_ loadingText : String?, gifimagename: String?)
    {
        ...
        if let overlayvw = UIWindow.key {
            let overlay = UIView(frame: overlayvw.frame)
            overlay.center = overlayvw.center
            overlay.alpha = 0
            overlay.backgroundColor = UIColor(named: "#262626") //.black
            ...
            imageView.frame = CGRect(x: 20.0, y: 50.0, width: 80, height: 80)
            overlay.addSubview(imageView)
            imageView.center = overlay.center;
        //overlayvw.backgroundColor = UIColor(named: "#551a8b")
            
            let label = UILabel()
            if let textString = loadingText
            {
                label.text = textString
                label.textColor = UIColor(red: 0.0/255, green: 181.0/255, blue: 152.0/255, alpha: 1.0)
                label.font = FontWithSize("Verdana", 14)
                label.sizeToFit()
                label.center = CGPoint(x: imageView.center.x, y: imageView.center.y + 50)
    ...
    public func showWithActivityIndicator(_ loadingText:String?, activitycolor: UIColor, labelfontcolor:UIColor , labelfontsize: Int,activityStyle: UIActivityIndicatorView.Style)
    {
        ...
        overlay.center = overlayvw.center
        overlay.alpha = 0
        overlay.backgroundColor = UIColor(named: "#262626")
        overlayvw.addSubview(overlay)
        overlayvw.bringSubviewToFront(overlay)
        ...
        overlayvw.backgroundColor = UIColor(named: "#262626")
